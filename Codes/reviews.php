...
try {
    $stmt = $pdo->prepare("SELECT DISTINCT w.worker_id, w.name FROM localhand3.bookings b JOIN localhand3.workers w ON b.worker_id = w.worker_id WHERE b.user_id = ?");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit_feedback'])) {
    ...
    if (!empty($worker_id) && !empty($rating) && !empty($comment)) {
        try {
            $stmt = $pdo->prepare("INSERT INTO localhand3.reviews (user_id, worker_id, rating, review_text, created_at) VALUES (?, ?, ?, ?, NOW())");
...
$feedback_history = [];
try {
    $stmt = $pdo->prepare("SELECT r.rating, r.review_text, r.created_at, w.name AS worker_name FROM localhand3.reviews r JOIN localhand3.workers w ON r.worker_id = w.worker_id WHERE r.user_id = ? ORDER BY r.created_at DESC");
    $stmt->execute([$user_id]);
