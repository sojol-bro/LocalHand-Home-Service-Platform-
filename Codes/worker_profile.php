...
$latest_portfolio_query = $pdo->prepare("
    SELECT photo_url 
    FROM portfolio 
    WHERE worker_id = ? 
    ...
    LIMIT 1
");
$latest_portfolio_query->execute([$worker_id]);
$latest_portfolio = $latest_portfolio_query->fetch(PDO::FETCH_ASSOC);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($section === 'info') {
        ...
        $sql = "UPDATE workers SET name = ?, phone = ?, email = ?, address = ?, specialty = ?, experience_years = ?,bio=?, hourly_rate = ? WHERE worker_id = ?";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$name, $phone, $email, $address, $specialty, $experience_years,$bio, $hourly_rate, $worker_id]);
...
<div class="container-fluid">
    ...
    <div>
    <div>
        <?php if (!empty($latest_portfolio['photo_url'])): ?>
            <img src="<?= htmlspecialchars($latest_portfolio['photo_url']) ?>" alt="Profile Photo" class="profile-photo">
        <?php else: ?>
            <img src="default.avif" alt="Default Profile Photo" class="profile-photo">
        <?php endif; ?>
        ...
        <h4><?= htmlspecialchars($worker_data['specialty']) ?></h4>
            ...
            <?php if ($section === 'info'): ?>
                ...
                <form method="POST">
                    ...
                    <div class="form-group">
                        <label>Bio:</label>
                        <input type="text" name="specialty" value="<?= htmlspecialchars($worker_data['bio']) ?>">
                    </div>
                    <div class="form-group">
